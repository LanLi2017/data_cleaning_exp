...
def run():
    ...
    parser.add_argument('-j', '--json_data', default=None,
                        help="JSON data: ORMA analysis on existing recipe")
    parser.add_argument('-s', '--schema_info', default=None,
                        help="Schema information: ORMA analysis on existing JSON recipe")
    ...
    if pas_req:
        try:
            ...
            orma_proc.generate_views(output=argobj.pop('output'), project_id=argobj.pop('input'), 
                                             type=argobj.pop('type'), **argobj)
        #     if argobj["type"] == "table_view":
        #         if argobj['combined']:
        #             orma_proc.generate_views(output=argobj["output"], project_id=argobj["input"], 
